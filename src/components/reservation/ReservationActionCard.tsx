...
const ReservationActionCard = memo(
  ({
    ...
    >
      ...
      <p className={`mb-4 text-sm leading-7 ${variant === 'green' ? 'text-white/75' : 'text-white/65'}`}>
        {description}
      </p>
      <p className={`mt-auto text-xl font-black ${variant === 'green' ? 'text-white' : 'text-amber-400'}`}>
        {actionText}
      </p>
    </motion.a>
  )
